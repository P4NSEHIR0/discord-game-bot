...
/*
            ...
            if (message.channel.id == this.client.config.channels.generalChat && !message.author.bot) {
                randomCode++;
              if (randomCode == 2) {
                randomCode = 0;
                let canCode = code[Math.floor(Math.random() * code.length)];
                ...
                ctx.fillText(`${canCode}`, 170, 285)
        
                const sabitle = new MessageAttachment(canvas.toBuffer(), "yazkazan.png")
                message.channel.send(`Aşadağıda verilen resimdeki kodu ilk sen yaz ve hediye coin kazan. HIZLI OL!!!\n`, sabitle)
            
    
                const coinOnay = await message.channel.awaitMessages((m) => [canCode].some(cevap => m.content.includes(cevap)), {max: 1, time: 15000 })
                if(coinOnay.size < 1 ) {return message.channel.send(`15 saniye içerisinde kimse cevap veremedeği için işlem iptal oldu.`).then(x => x.delete({timeout:7500}))}
                let cevaplama = coinOnay.first()
                if(cevaplama.content.includes(canCode)) {
                    let randomizeCoin = Math.floor(Math.random() * 450) + 1
        
                    let data = await oyun.findOne({sunucu: message.guild.id, user: message.author.id})
        
                    if(!data) { 
                    let newData = new oyun({sunucu: message.guild.id, user: message.author.id, coin: randomizeCoin})
                    newData.save().catch(err => console.log(`Oyuncuza yaz kazandan para eklenemedi (yeni data açılamadı) !\nSorun:` + err))
                    } else {data.sunucu = message.guild.id;data.user = message.author.id; data.coin = (data.coin + randomizeCoin); data.save();}
        
                    message.channel.send(new MessageEmbed().setAuthor(message.guild.name, message.guild.iconURL({dynamic: true, size: 2048}))
                    .setDescription(`${message.author} kazandı, tam **${randomizeCoin}** coin kazandın tebrik ederim ! \nYeni bakiyeni kontrol etmek için`))
